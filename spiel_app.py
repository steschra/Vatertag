    ...
    for runden_index, runde in enumerate(st.session_state.runden[::-1]):
        ...
        with st.expander(f"{runde['name']}", expanded=(runden_index == 0)):
            ...
            for sp in st.session_state.spieler:
                ...
                runde["einsaetze"][sp["name"]] = st.number_input(f"{sp['name']}: Einsatz", min_value=0.0, value=runde["einsaetze"].get(sp["name"], 0.0), key=key)

            if st.button(f"Weiter zu Platzierung - Runde {echte_index+1}", key=f"weiter_{echte_index}"):
                st.rerun()
